...
def main(args):
    ...
    for metric_type in metric_types.split(","):
        ...
        if export_dir:
            ...
            if metric_type == 'WB_PESQ':
                write_to_txt(export_dir + str(metric_type) + "_list.txt", metrics_result_store)
            write_to_txt(export_dir + str(metric_type) + ".txt",[ ('total', metric_value)])

            # import tablib

            # export_path = export_dir / f"{metric_type}.xlsx"
            # export_path = export_dir / f"{metric_type}.csv"
