...
def entry(rank, world_size, config, resume, only_validation, port):
    ...
    s = socket()
    s.bind(("", 0))
    os.environ["MASTER_PORT"] = port  # A random local port
...
#    if 'Conformer' in config["model"]["args"]['sequence_model']:
 #       scheduler= torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.001, steps_per_epoch=len(train_dataset), epochs=config['trainer']['train']['epochs'])
#    else:
    scheduler= None
...
if __name__ == '__main__':
    ...
    parser.add_argument("--port", default='1111', type=str)
    ...
    mp.spawn(entry,
             args=(args.num_gpus, configuration, args.resume, args.only_validation, args.port),
