...
class BaseInferencer:
    ...
    def __call__(self):
        ...
        total_rtf, total_model_rtf = 0, 0
        for noisy, name in tqdm(self.dataloader, desc="Inference"):
            assert len(name) == 1, "The batch size of inference stage must 1."
            name = name[0]

            t1 = time.time()
            enhanced, model_rtf = getattr(self, inference_type)(noisy.to(self.device), inference_args)
            t2 = time.time()
            ...
            tqdm.write(f"{name}, rtf: {rtf}")

            total_model_rtf += model_rtf
            total_rtf += rtf
        ...
        print(f'avg model rtf: {total_model_rtf / len(self.dataloader)}')
        print(f'avg rtf: {total_rtf / len(self.dataloader)}')
