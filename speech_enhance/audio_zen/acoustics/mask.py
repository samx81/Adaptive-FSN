...
def build_ideal_ratio_mask(noisy_mag, clean_mag) -> torch.Tensor:
    ...
    assert not ratio_mask.isnan().any(), ratio_mask.isnan()
    ratio_mask = ratio_mask[..., None]
     
    ret = compress_cIRM(ratio_mask, K=10, C=0.1)
    assert not ret.isnan().any(), ret.isnan()

    return ret

def build_ideal_ratio_mask_general(noisy_mag, clean_mag) -> torch.Tensor:
    """
