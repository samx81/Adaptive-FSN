...
from fullsubnet_plus.model.manner_blocks import ResConBlock, Expendable_ResConBlock
from fullsubnet_plus.model.conformer import ConformerBlock
...
class transpose_last_two(nn.Module):
    def forward(self, x):
        return x.transpose(-1, -2)

class reshape_freq(nn.Module):
    def __init__(self, F_size, module):
        super().__init__()
        self.F_size = F_size
        self.module = module
    ...
    def forward(self, x):
        _, sb, t = x.size()
        x = x.reshape(-1, self.F_size, sb, t)
        x = self.module(x)
        x = x.reshape(-1, sb, t)
        return x
...
class SequenceModel(nn.Module):
    def __init__(
        ...
        elif self.sequence_model_type == "Conformer":
            if isinstance(num_layers, list):
                self.sequence_model = nn.Sequential(
                    *[Expendable_ResConBlock(input_size, kernel_size=conformer_kernel, growth1=hidden_size, growth2=1, causal=True, dilation=i, favor_attention=True) for i in num_layers],
                    nn.ReLU()
                )
            elif isinstance(conformer_kernel, list):
                self.sequence_model = nn.Sequential(
                    *[Expendable_ResConBlock(input_size, kernel_size=k, growth1=hidden_size, growth2=1, causal=True, dilation=1, favor_attention=True) for k in conformer_kernel],
                    nn.ReLU()
                )
            else: 
                # WARNING: Old, legacy module!
                # Only to present in paper
                self.sequence_model = nn.Sequential(
                    *[ResConBlock(input_size, growth1=2, growth2=1, causal=True, dilation=pow(2, i), favor_attention=True) for i in range(num_layers)], #pow(2, i)  # if i == 0 else False
                    nn.ReLU()
                )
    ...
    def forward(self, x, batch_size = None):
        ...
        Returns:
            [B, F, T]
        """
        if self.sequence_model_type in ["TCN", "TCN-subband", "Conformer"]:
            assert x.dim() == 3
