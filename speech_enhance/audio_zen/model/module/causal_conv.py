...
class GatedTCNBlock(nn.Module):
    def __init__(self, in_channels=257, hidden_channel=512, out_channels=257, kernel_size=3, dilation=1,
        ...
        self.norm_gate = nn.GroupNorm(1, hidden_channel, eps=1e-8)
        self.depthwise_conv_gate = nn.Conv1d(hidden_channel, hidden_channel, kernel_size=kernel_size, stride=1,
                                        groups=hidden_channel, padding=padding, dilation=dilation)
    ...
    def forward(self, x):
        """
            x: [channels, T]
        """
        y_pre = self.conv1x1(x)
        y = self.norm1(self.prelu1(y_pre))
        y = self.depthwise_conv(y)

        y_gate = self.prelu_gate(y_pre)
        y_gate = self.norm_gate(y_gate)
        y_gate = self.depthwise_conv_gate(y_gate)
        y_gate = self.sigmoid(y_gate)
        y = y * y_gate
        ...
        y = self.norm2(self.prelu2(y))
        output = self.sconv(y)
        return x + output if self.use_skip_connection else output
