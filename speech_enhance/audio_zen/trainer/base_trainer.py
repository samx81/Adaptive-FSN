...
class BaseTrainer:
    def __init__(self, dist, rank, config, resume, only_validation, model, loss_function, optimizer):
        ...
        if self.rank == 0 or self.dist == None:
    ...
    def metrics_visualization(self, noisy_list, clean_list, enhanced_list, metrics_list, epoch, num_workers=10, mark=""):
        ...
        for metric_name in metrics_list:
            ...
            if metric_name == "WB_PESQ":
                wb_pesq_ori = mean_score_on_enhanced
                wb_pesq_mean = transform_pesq_range(mean_score_on_enhanced)
        log(f'STOI: {stoi_mean}, PESQ: {wb_pesq_ori}')
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.only_validation and (self.rank == 0 or self.dist == None):
            ...
            if (self.rank == 0 or self.dist == None) and self.save_checkpoint_interval != 0 and (epoch % self.save_checkpoint_interval == 0):
                self._save_checkpoint(epoch)

            # Regular validation
            if (self.rank == 0 or self.dist == None) and (epoch % self.validation_interval == 0):
                print(f"[{timer.duration()} seconds] Training has finished, validation is in progress...")

                self._set_models_to_eval_mode()
                metric_score = self._validation_epoch(epoch)
                print(f'current:{metric_score}, best:{self.best_score}')
