...
from fullsubnet_plus.model.manner_blocks import SubbandEncoder, SubbandEncoder_submain, GTU1d, \
    SubbandEncoder_FullCausal, SubbandEncoder_FullCausal_bottomup, SubbandEncoder_submain_causal
from fullsubnet_plus.model.phase_enc import PhaseEncoder 
...
class FullSubNet_Plus(BaseModel):
    def __init__(self,
        ...
        """
        super().__init__()
        # assert sequence_model in ("GRU", "LSTM", "TCN", "Conformer",'LSTM-Conformer', 'Normal-Conformer', '357Conformer'), f"{self.__class__.__name__} only support GRU, LSTM and TCN."
        ...
        self.sb_sequence_model_type = sequence_model

        if weight_init:
            self.apply(self.weight_init)

    def forward(self, noisy_mag, noisy_real, noisy_imag, dropband=True):
        """
        Args:
        ...
        if self.sb_sequence_model_type == 'TFCM_ASA':
            sb_input = sb_input.transpose(1, 2) # [B, (F_s + 3 * F_f), F, T]
            sb_mask = self.sb_model(sb_input, batch_size)  # [B, F, 2, T]

            if dropband and batch_size > 1:
                sb_mask = drop_band(sb_mask, num_groups=self.num_groups_in_drop_band)  # [B, (F_s + F_f), F//num_groups, T]

            # sb_mask = sb_mask.transpose(1, 2)
...
class FullSubNet_MANNER_Finetune(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
                 ...
                 kersize=[3, 5, 10],
                 weight_init=True,
                 subband_num_freq=257,
                 subband_kernel = 8,
                 subband_stride = 4,
                 subband_depth = 2,
                 sb_num_layers=2,
                 conformer_kernel=31,
                 ada_subband_model='causal'
                 ):
        """
        FullSubNet model (cIRM mask)
        ...
        """
        super().__init__()
        # assert sequence_model in ("GRU", "LSTM", "TCN", 'Conformer','Conformer_ASA'), f"{self.__class__.__name__} only support GRU, LSTM and TCN."
        ...
        if channel_attention_model:
            ...
            print(f"Not implemented channel attention model {self.channel_attention}")
            # raise NotImplementedError(f"Not implemented channel attention model {self.channel_attention}")
        ...
        # subband encoder
        if ada_subband_model == 'subcausal':
            self.subband_enc = SubbandEncoder_submain_causal(num_freqs, subband_num_freq, kernel_size=subband_kernel, stride=subband_stride, depth=subband_depth, head=1)
        else:
            self.subband_enc = SubbandEncoder_FullCausal_bottomup(num_freqs, subband_num_freq, kernel_size=subband_kernel, stride=subband_stride, depth=subband_depth, head=1)

        self.sb_sequence_model_type = sequence_model
        self.sb_model = SequenceModel(
            #input_size=(sb_num_neighbors * 2 + 1) + 3 * (fb_num_neighbors * 2 + 1),
            input_size=self.subband_enc.next_subband + 3 * (fb_num_neighbors * 2 + 1),
            output_size=output_size,
            hidden_size=sb_model_hidden_size,
            num_layers=sb_num_layers,
            bidirectional=False,
        ...
        self.subband_num = subband_num
        # self.sb_num_neighbors = sb_num_neighbors
        self.sb_unfold_freqs = subband_num_freq
        self.sb_num_neighbors = self.subband_enc.next_subband
        self.fb_num_neighbors = fb_num_neighbors
        self.look_ahead = look_ahead
        self.norm = self.norm_wrapper(norm_type)
        self.num_groups_in_drop_band = num_groups_in_drop_band
    ...
    def forward(self, noisy_mag, noisy_real, noisy_imag, dropband=True):
        ...
        # Unfold attention noisy input, [B, N=F, C, F_s, T]
        noisy_mag_unfolded = self.unfold(fb_input.reshape(batch_size, 1, num_freqs, num_frames),
                                         num_neighbor=self.sb_unfold_freqs // 2)
        noisy_mag_unfolded = noisy_mag_unfolded.reshape(batch_size, num_freqs, self.sb_unfold_freqs,
                                                        num_frames)
        # print(noisy_mag_unfolded.shape, self.subband_enc)                                                        
        noisy_mag_unfolded = self.subband_enc(noisy_mag_unfolded)
        ...
        if self.sb_sequence_model_type == 'TFCM_ASA':
            ...
            sb_input = sb_input.reshape(
                batch_size * num_freqs,
                (self.sb_num_neighbors) + 3 * (self.fb_num_neighbors * 2 + 1),
                num_frames
            )

            # [B * F, (F_s + F_f), T] => [B * F, 2, T] => [B, F, 2, T]
            sb_mask = self.sb_model(sb_input, batch_size)
            sb_mask = sb_mask.reshape(batch_size, num_freqs, self.output_size, num_frames).permute(0, 2, 1, 3).contiguous()
...
class FullSubNet_MANNER_Mag_Bottomup(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
        ...
        """
        super().__init__()
        assert sequence_model in ("GRU", "LSTM", "TCN", 'Conformer'), f"{self.__class__.__name__} only support GRU, LSTM and TCN."
        ...
        self.sb_model = SequenceModel(
            #input_size=(sb_num_neighbors * 2 + 1) + 3 * (fb_num_neighbors * 2 + 1),
            input_size=self.subband_enc.next_subband + 1 * (fb_num_neighbors * 2 + 1),
            output_size=output_size,
            hidden_size=sb_model_hidden_size,
            num_layers=sb_num_layers,
            bidirectional=False,
    ...
    def forward(self, noisy_mag, noisy_real, noisy_imag, dropband=True):
        ...
        sb_input = sb_input.reshape(
            batch_size * num_freqs,
            (self.sb_num_neighbors) + 1 * (self.fb_num_neighbors * 2 + 1),
            num_frames
        )
...
class FullSubNet_MANNER_Mag(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
        ...
        # subband encoder
        self.subband_enc = SubbandEncoder_FullCausal(num_freqs, subband_num_freq, kernel_size=subband_kernel, stride=subband_stride, depth=subband_depth, head=1)
...
class FullSubNet_Adaptive(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
        ...
        """
        super().__init__()
        assert sequence_model in ("GRU", "LSTM", "TCN", 'Conformer'), f"{self.__class__.__name__} only support GRU, LSTM, Conformer and TCN."

        if subband_num == 1:
            self.num_channels = num_freqs
        else:
            self.num_channels = num_freqs // subband_num + 1
        self.channel_attention_model = channel_attention_model
        if channel_attention_model:
            if channel_attention_model == "SE":
                self.channel_attention = ChannelSELayer(num_channels=self.num_channels)
                self.channel_attention_real = ChannelSELayer(num_channels=self.num_channels)
        ...
        # subband encoder
        self.subband_enc = SubbandEncoder_submain(num_freqs, subband_num_freq, kernel_size=subband_kernel, stride=subband_stride, depth=subband_depth, head=1)
